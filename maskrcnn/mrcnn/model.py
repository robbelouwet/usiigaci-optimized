...
from maskrcnn.mrcnn.parallel_model import ParallelModel
...
def load_image_gt(dataset, config, image_id, augment=False, augmentation=None,
    ...
    if augmentation:
        ...
        warnings.filterwarnings("ignore", category=imgaug.augmenters.SuspiciousSingleImageShapeWarning)
...
# https://github.com/matterport/Mask_RCNN/pull/1611
...
#Next data_generator function is deprecated in favour of the previous Data_generator class
...
class EarlyStoppingByLossVal(tf.keras.callbacks.Callback):
    ...
    def on_epoch_end(self, epoch, logs={}):
        ...
        if current < self.value:
            if self.verbose > 0:
                log("Epoch %05d: early stopping THR" % epoch)
...
class MaskRCNN():
    ...
    def __init__(self, mode, config, model_dir, model_name):
        """
        ...
        model = self.build(mode=mode, config=config)
    ...
    def compile(self, learning_rate, momentum):
        ...
        for name in loss_names:
            ...
            res = False
            for item in self.keras_model.losses:
    ...
    def set_log_dir(self, model_path=None):
        ...
        self.log_dir = os.path.join(self.model_dir, "{}_{:%Y%m%dT%H%M}".format(
            self.model_name if self.model_name is not None else self.config.NAME.lower(), datetime.now()))
        print(f'> Logs will be stored at {self.log_dir}')

        # Path to save after each epoch. Include placeholders that get filled by Keras.
        self.checkpoint_path = os.path.join(self.log_dir, "mask_rcnn_{}_*epoch*_*val_loss*.h5".format(
    ...
    def train(self, train_dataset, val_dataset, learning_rate, epochs, layers,
        ...
        callbacks = [
            ...
            keras.callbacks.ModelCheckpoint(self.checkpoint_path, mode='min', monitor='val_loss',
                                            save_best_only=True, verbose=1, save_weights_only=True),
        ]

        # Learning rate scheduler
        # This function keeps the initial learning rate for the first 10% of epochs and decreases it after
        def scheduler(epoch, lr):
            if epoch > math.floor(self.config.EPOCHS_ALL * 0.10):
                if lr <= 0.0001:
                    lr = 0.0001
                else:
                    lr = lr * self.config.SCHEDULER_LR_DECREASER if self.config.SCHEDULER_LR_DECREASER is not None else 1

            log(
                f'Current learning rate at epoch {epoch + 1}: {lr}. Timestamp: {datetime.now().strftime("%Y-%m-%d %H:%M:%S")}')
        ...
        if custom_callbacks:
            ...
            # callbacks.append(EarlyStoppingByLossVal(monitor='val_loss', value=0.5, verbose=1))
        ...
        #   json_file.write(self.keras_model.to_json())
        # print(self.keras_model.to_json())
        # await self.keras_model.save('downloads://my-model')
